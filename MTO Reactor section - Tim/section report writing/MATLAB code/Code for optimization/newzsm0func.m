function outfunc=newzsm0func(p,t,f,cw)
clear all;
clf;
clc;
%% legend
...
dp=dp;%308*10^(-6);%m, particle diameter, x3 from literature because to improve
...
catalystweight=cw; %kg
reactorvolume=catalystweight/beddensity; %m3
H=reactorvolume*(4/pi)/d^2; %m, bed height
...
selectivity=[];
for iii=1:11
selectivity(iii)=100*totalmolarflow(n,iii)./(sum(totalmolarflow(n,:))-totalmolarflow(n,iii)-totalmolarflow(n,3)); %selectivity pertaining to olefins only
carbonpercent(iii)=100*totalmolarflow(n,iii)./(sum(totalmolarflow(n,:))-totalmolarflow(n,3));
end

%% matrix for storing in outside function
outfunc=zeros(1,26);
outfunc(1:11)=totalmassflow(n,:).*31.536;
outfunc(12)=exitingmassflow.*31.536;
...
outfunc(19)=H;
outfunc(20)=u0s(n);
outfunc(21)=selectivity(11);
outfunc(22)=selectivity(4);
outfunc(23)=selectivity(5);
outfunc(24)=selectivity(6);
outfunc(25)=selectivity(7);
outfunc(26)=selectivity(8);

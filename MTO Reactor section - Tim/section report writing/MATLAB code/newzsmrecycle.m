...
%% constants 
P= 3030000; %pressure constant of reactor, Pa, set at 30 atm to decrease volume flow
T=550+273.15; %temperature constant of reactor, K
n=200; %no of cstr stages
...
dp=180*10^(-6);%m, particle diameter, x3 from literature because too small
...
q0=27800/3600; %m3/s %stage 0 volumetric flow rate from packed bed hysys =24380 m3/h, 36500 is w/o recycle for 880kt/a capacity
...
%reactorvolume=(pi/4)*d^2*H;%m3, 
%catalystweight=beddensity*reactorvolume; %kg
massflow0=(molarflowmeth*32.0429+molarflowh20*18.0159)*3600 %kg/h
WHSV=134;
catalystweight=massflow0/WHSV %kg
bedvolume=catalystweight/beddensity; %m3
H=bedvolume*(4/pi)/d^2 %m, bed height
...
Vb=(1-emulfrac)*bedvolume;%m3, volume of bubble phase in reactor
Ve=emulfrac*bedvolume;%m3
...
while abs(Vr-Vrnew)>=0.005 %recycle loop
    herp2=herp2+1;
...
for a = 2:n; %no. of stages
       ...
       while abs(u0new-u0) > 0.001 %cant solve <500 iter if tolerance <0.001 at 300 cstr stgs
...
Vout=sum(Fout)*6130*T/P; Vrnew=sum(Fr)*6130*T/P; %ideal gas correlation causing problem ...
%(Vout>V2, as such 8314 is reduced to 6225 ensure V2=Vout+Vr)
...
%% checking for slugging and terminal velocity
leftshark=H/d;
rightshark=1.9/(rollp*dp)^0.3;
if leftshark-rightshark <=0
    fprintf('slugging will never occur')
else
    ums=umf+0.16*(1.34*d^0.175-H)^2+0.07*(9.81*d)^0.5;
    if ums-u0s(n)<=0
        fprintf('slugging will occur')
    else
        fprintf('slugging might occur, just not yet')
    end
end

if ut-u0s(n)<=0
    fprintf('fast fluidized regime reached')
else
    fprintf('bubbling fluidizing regime')
end
...
plantcapacity=c2c3massflow*28.8 %kt/a, assuming 8000 hours/annum

inletmethanolmassflow=molarflowmeth*mw(1)*28.8 %kt/a
inletmassflow=molarflowmeth*mw(1)*28.8+molarflowh20*mw(3)*28.8

TDH=1000*(u0s(n)^2)/9.81
reactorheight=H+TDH
reactorvolume=pi*reactorheight*(d/2)^2

regenH2inletkta=0.5*molarflow0*2*28.8
...
u0s(n)
ut
maxq0=area*ut % max volumetric flow rate for this given diameter before fast fluidize
q0 % comparing initial flow rate with max flow rate
end

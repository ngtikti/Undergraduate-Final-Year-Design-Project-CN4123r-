...
%% constants 
P= 3000000; %pressure constant of reactor, Pa, set at 30 atm to decrease volume flow
T=550+273.15; %temperature constant of reactor, K
n=200; %no of cstr stages
%H=1; %m, total height of reactor, increase height, increase catalyst,
...
q0=25350/3600; %m3/s %stage 0 volumetric flow rate from packed bed hysys =24380 m3/h
...
%% finding molar, mass flow and component fractions
   if herp==500
        derp
        fprintf('Cant solve within 500 iterations')
        u0
        u0new
        return
   end 
...
exitingmolarflow=sum(totalmolarflow(n,:))
exitingmassflow=sum(totalmassflow(n,:))

compmolefrac = totalmolarflow(n,:)/sum(totalmolarflow(n,:))
compmassfrac=totalmassflow(n,:)/sum(totalmassflow(n,:))

compmolarflow=totalmolarflow(n,:)
compmassflow=totalmassflow(n,:)
%% graph plot

figure;
plot(totalmolarflow(1:n,4:11))
title('Molar flow profile')
legend('Ethene','Propene','Butene','Pentene','Hexene','Cx','C','CH4')


%% Additional calculations
meohconversion=(totalmolarflow(1,1)-totalmolarflow(n,1))*100/totalmolarflow(1,1) % percent
...
molarflow0
massflowmeth0=molarflowmeth*mw(1)
d
catalystweight
db;
dbs;
